...
- **Real-world test assets (e.g. Sponza)** -- no mesh loader (OBJ/glTF parser) or texture loading exists, and Sponza's texture-based materials would need both to look meaningfully different from the procedural scene, which is real added engineering (parser, texture pipeline, multi-material support) beyond just importing a file. More importantly, it's not needed: the procedural grid scene is *parametrically scalable* (light count, object count, both swept smoothly from 1 to thousands), which a fixed-complexity scene like Sponza is not -- that scalability is exactly what the controlled crossover-point methodology depends on. A real GPU crossover was already observed on the procedural scene (Forward ~1.4x Deferred's GPU time at 64 lights / ~2000 objects), confirming the synthetic scene is sufficient to answer the research question. Sponza would be a one-off validation appendix at best, not a replacement for the sweep methodology. Discuss as future work, don't build, unless every other phase finishes early.
...
First real result: at a heavy manual test (64 lights, ~2000 objects), Deferred's total GPU time (~3.6ms) was clearly below Forward's (~4.9-5.2ms) -- the predicted crossover, actually observed. CPU/FPS numbers stayed identical between modes at that scale (vsync-capped, both under budget), confirming GPU per-pass timing is the signal that matters, not CPU frame time, until a pipeline gets slow enough to blow the vsync budget itself.

Four numbered presets (1-4: Low/Medium/High/Extreme) now snap light count, object count, and camera pose to fixed combos, independent of the Tab render-mode toggle -- press a preset once, then Tab between Forward/Deferred to compare the exact same frozen configuration instead of a manually-held, drifting one.

## Next step
Forward Rendering and Deferred Shading are both implemented and toggleable at runtime (Tab key), `Scene` is procedurally configurable (Up/Down = light count, Left/Right = object count, or presets 1-4), and both pipelines report live GPU per-pass timing + CPU frame time (title bar and console, updated twice/sec; V toggles vsync). Next: implement Visibility Buffer -- visibility pass writing packed triangle/object IDs, resolve pass doing vertex-pulling via SSBOs, shading pass reusing `evaluatePointLight()`. The benchmark *harness* itself (programmatic sweeps + CSV export, vs. a human holding arrow keys) is still Week 7 work, not yet started.
